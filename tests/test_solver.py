...
def _registry_setting(operation: RegistryOperation) -> Setting:
    return Setting(
        SettingKind.REGISTRY,
        "Software\\Policies\\Example\\Danger",
        {"type": 4, "data": 1},
        dangerous=True,
        ...
        registry_value_name="Danger",
    )


def _registry_env(operation: RegistryOperation):
    danger = dangerous_gpo(
        machine_extensions=(REGISTRY_CSE_GUID,),
        settings=(_registry_setting(operation),),
    )
    return environment(danger=danger, ou_sd=som_sd(GPLINK_GUID), include_safe=False)


def test_set_if_absent_registry_is_a_coverage_gap_not_a_deterministic_finding() -> None:
    solver = CounterfactualSolver(_registry_env(RegistryOperation.SET_IF_ABSENT))
    findings = solver.solve()
    assert findings == []
    gates = {gap.gate for gap in solver.coverage_gaps}
    assert "TARGET_LOCAL_STATE" in gates


def test_set_value_registry_still_produces_a_finding() -> None:
    solver = CounterfactualSolver(_registry_env(RegistryOperation.SET_VALUE))
    findings = solver.solve()
    assert findings
    assert findings[0].actions[0].type is ActionType.ADD_LINK
    assert "TARGET_LOCAL_STATE" not in {gap.gate for gap in solver.coverage_gaps}
